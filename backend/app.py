...
def align_text_mfa(input_path, output_dir="output"):
    """
    Aligns the transcript with the audio using Montreal Forced Aligner (MFA)
    running in the 'align_text_mfa' conda environment.
    """
    command = [
        # Run in the align_text_mfa environment
        "conda", "run", "-n", "aligner",
        "mfa", "align", "--clean", "--verbose", input_path,
        "english_us_arpa", "english_us_arpa", output_dir
    ]
...
def process_article():
    ...
    with the same filename, saves the generated image to a new folder called 'images',
    and then runs MFA alignment on the 'process_transcription' folder.
    ...
    # Save the generated image to a new folder called 'images' with the same filename.
    images_dir = "images"
    ...
    try:
        align_text_mfa(input_path=transcription_dir,
